...
sub get_jwt_signing_key {
	...
	if ( $hr->{external_id} ) {
		...
		if ( !$args ) {
			warn "Temporary issue errors: ", Dumper( \@errors );
			die $self->graceful_error(
...
sub authenticate_account($$$) {
	...
	if ( !( $sth->execute() ) ) {
		...
		warn "Temporary issues authenticating user: ", $msg;
...
sub dbh($) {
	...
	if ( !exists( $self->{_dbh} ) ) {
		...
		if ( !$dbh ) {
			warn "database error: ", $JazzHands::DBI::errstr, "\n";
			die $self->graceful_error( {
...
sub mint_jwt($) {
	...
	if ( !$key ) {
		warn
		  "There appears to be no JWT signing key in the database for $scope";
	...
	# Emit debugging to container output
	warn $j->pretty->encode($jwtbase);
