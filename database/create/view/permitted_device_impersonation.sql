...
--
-- This covers if a user is able to act as a device
--
CREATE OR REPLACE VIEW jazzhands_openid.permitted_device_impersonation AS
SELECT	DISTINCT service_endpoint_uri,
	lhs.login,
	rhs.device_name as actas
FROM jazzhands.property
	...
	JOIN (
		SELECT root_device_collection_id AS property_value_device_collection_id,
	...
	) rhs USING (property_value_device_collection_id)
WHERE property_type = 'jazzhands-openid'
AND property_name = 'act-as-device'
;
